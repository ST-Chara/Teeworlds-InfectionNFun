...
int IGameController::PickHero() {

	for (int i = 0; i < MAX_CLIENTS; i ++) {
        CPlayer *pPlayer = GameServer()->m_apPlayers[m_NextHero];
        if (!pPlayer) {
        ...
        if (pPlayer->GetTeam() == TEAM_SPECTATORS || pPlayer->Infected() || m_NextHero == m_NextZombie) {
            if (++ m_NextHero >= MAX_CLIENTS)
                m_NextHero = 0;
            continue;
        }
